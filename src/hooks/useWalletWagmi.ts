...
import { megaEthTestnet } from '../providers/WagmiProvider';
...
export function useWalletWagmi() {
  ...
  const { disconnect: disconnectWagmi, status: disconnectStatus } = useDisconnect();
  ...
  const isPending = connectStatus === 'pending' || disconnectStatus === 'pending';

  // Function to fetch session key balance
  const fetchSessionBalance = useCallback(async (sAddress: `0x${string}`) => {
  ...
  useEffect(() => {
    if (isConnected && address && publicClient) { // Ensure publicClient is available too
      ...
      try {
        ...
        console.log(`Using session key ${currentSessionAddress} for ${address}.`);
        ...
        setSessionBalance('0');
        setError('Failed to initialize session key. Please disconnect and reconnect.');
      }
  ...
  const refetchSessionBalance = useCallback(async () => {
    ...
    try {
      setIsFetchingBalance(true);
      const balance = await publicClient.getBalance({ address: sessionAddress });
